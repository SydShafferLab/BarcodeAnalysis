...
#path to all barcode analysis scripts
scripts="/Users/dylanschaff/Documents/GitHub/BarcodeAnalysis"

# Determine whether the data has barcodes from 10x and gDNA "both" or only from 10x "10x"
barcodeSource = "both"

#Folder that contains all folders containing 10xcDNA FASTQ files generated from sequencing the barcodes
Fastqfolder10x="/Users/dylanschaff/Documents/GitHub/BarcodeAnalysis/Extract10xBarcodes/TestData/test_cDNA_fastqs"

#Folder that contains all folders containing gDNA FASTQ files generated from sequencing the barcodes. If you do not have corresponding gDNA sample, leave empty ('').
FastqfoldergDNA = '/Users/dylanschaff/Documents/GitHub/BarcodeAnalysis/Extract10xBarcodes/TestData/test_gDNA_fastqs'

#folder you want outputs go go into (dont make this folder, this scipt will make it)
Outfolder = "/Users/dylanschaff/Desktop/Test_BC_Pipeline"

#length to keep from sequenced barcode (this is actual barcode, does not include strt seq below)
bclen = 70
...
#NOTE: names used here should match the names that will be assinged to samples(should be everythibg before lane (_L0))
#NOTE: If running barcodes from both 10x cDNA and gDNA from the same groups, list all 10x samples followed by all gDNA samples. 
    #EXAMPLE: [["s1_10xcDNA","s2_10xcDNA","s1_gDNA","s2_gDNA"]]
GSAMP = [["Test1_10xcDNA", "Test1_gDNA"],["Test2_10xcDNA", "Test2_gDNA"]]

#### END OF USER DEFINED PARAMETERS ##### NOTE: there are many more parameters that can be changed but this requires you to go into the code below and understand all the different funtions
...
if not os.path.exists(Outfolder):
    os.mkdir(Outfolder)
    print("folder '{}' created ".format(Outfolder))
else:
    print("folder {} already exists".format(Outfolder))

if not os.path.exists(sc_in):
    os.mkdir(sc_in)
    print("folder '{}' created ".format(sc_in))
else:
    print("folder {} already exists".format(sc_in))

if not os.path.exists(sc_out):
    os.mkdir(sc_out)
    print("folder '{}' created ".format(sc_out))
else:
    print("folder {} already exists".format(sc_out))
    
if not os.path.exists(mod_R2):
    os.mkdir(mod_R2)
    print("folder '{}' created ".format(mod_R2))
else:
    print("folder {} already exists".format(mod_R2))
    
if not os.path.exists(filt_haveStart_10x):
    os.mkdir(filt_haveStart_10x)
    print("folder '{}' created ".format(filt_haveStart_10x))
else:
    print("folder {} already exists".format(filt_haveStart_10x))
    
if barcodeSource == 'both':
    if not os.path.exists(filt_haveStart_gDNA):
        os.mkdir(filt_haveStart_gDNA)
        print("folder '{}' created ".format(filt_haveStart_gDNA))
    else:
        print("folder {} already exists".format(filt_haveStart_gDNA))
   
if not os.path.exists(filt_WSN_10x):
    os.mkdir(filt_WSN_10x)
    print("folder '{}' created ".format(filt_WSN_10x))
else:
    print("folder {} already exists".format(filt_WSN_10x))
   
if barcodeSource == 'both':
    if not os.path.exists(filt_WSN_gDNA):
        os.mkdir(filt_WSN_gDNA)
        print("folder '{}' created ".format(filt_WSN_gDNA))
    else:
        print("folder {} already exists".format(filt_WSN_gDNA))
   
if not os.path.exists(CellR):
    os.mkdir(CellR)
    print("folder '{}' created ".format(CellR))
else:
    print("folder {} already exists".format(CellR))
    
if not os.path.exists(CellRfq):
    os.mkdir(CellRfq)
    print("folder '{}' created ".format(CellRfq))
else:
    print("folder {} already exists".format(CellRfq))
